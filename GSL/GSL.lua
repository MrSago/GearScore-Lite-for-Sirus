    ...
    if (GS_ItemTypes[ItemEquipLoc]) then
        ...
        if (ItemRarity >= 2) and (ItemRarity <= 4) then
            local Red, Green, Blue = GearScore_GetQuality((floor(
                ((ItemLevel - Table[ItemRarity].A) / Table[ItemRarity].B) * 1 * Scale)) * 11.75)
    ...
    if (MouseOverGearScore) and (MouseOverGearScore > 0) and (GS_Settings["Player"] == 1) then
        ...
        if (GS_Settings["Compare"] == 1) then
            ...
            if (MyGearScore > TheirGearScore) then
                GameTooltip:AddDoubleLine("YourScore: " .. MyGearScore, "(+" .. (MyGearScore - TheirGearScore) .. ")",
                    0, 1, 0, 0, 1, 0);
            end
            if (MyGearScore < TheirGearScore) then
                GameTooltip:AddDoubleLine("YourScore: " .. MyGearScore, "(-" .. (TheirGearScore - MyGearScore) .. ")",
                    1, 0, 0, 1, 0, 0);
            end
            if (MyGearScore == TheirGearScore) then
...
PaperDollFrameEquipInset:HookScript("OnShow", MyPaperDoll)

local pgs = CreateFrame("Frame", "PGS", PaperDollFrameEquipInset)
pgs:SetFrameStrata("HIGH")
pgs:SetHeight(29)
pgs:SetWidth(167)
pgs:SetPoint("BOTTOM", "PaperDollFrameEquipInset", "BOTTOM", 93, 66)
pgs:Show()

pgs:CreateFontString("PersonalGearScore")
PersonalGearScore:SetFont("Fonts\\FRIZQT__.TTF", 16)
PersonalGearScore:SetText("0")
PersonalGearScore:SetPoint("CENTER", pgs, "CENTER", 0, 0)
